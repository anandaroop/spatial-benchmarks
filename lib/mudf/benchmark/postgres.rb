...
module MUDF
  module Benchmark
    class Postgres < Base
      ...
      ENVELOPE_QUERY_INDEXED = <<~SQL
        SELECT count(*)
        FROM orgs
        WHERE location_indexed && ST_MakeEnvelope($1, $2, $3, $4, 4326);
      SQL
      ...
      def initialize
        ...
        @client.prepare('location_indexed_envelope', ENVELOPE_QUERY_INDEXED)
      end

      def run
        each_bounding_box do |box|
          result = @client.exec_prepared('location_indexed_envelope',
